...
the_result <- paste0(res_dir,"moz_lulc2016_20180406.tif")
...
system(sprintf("python %s/s7_misc/oft-zonal_large_list.py -i %s -um %s -o %s -a %s",
               ...
               the_result,
               all_poly,
               paste0(res_dir,"zonal_training_result_20180406.txt"),
               "poly_id"
))
...
write.csv(df3,paste0(res_dir,"map_vs_ref_val_poly_20180406.csv"),row.names = F)
...
write.csv(areas_merge,paste0(res_dir,"areas_l2_20180406.csv"),row.names = F)
...
write.csv(areas_l1,paste0(res_dir,"areas_l1_20180406.csv"),row.names = F)
