...
####################################################################################################
####################################################################################################
train_dir  <- "/media/dannunzio/OSDisk/Users/dannunzio/Documents/countries/mozambique/training_data/"
library(foreign)
...
system(sprintf("python %s/s7_misc/oft-zonal_large_list.py -i %s -um %s -o %s -a %s",
               ...
               the_result,
               all_poly,
               paste0(res_dir,"zonal_training_result_20171208.txt"),
               "poly_id"
))


#### SELECT ONLY COLUMNS OF INTEREST< COMPUTE THE MODE AND THE PURITY OF EACH POLYGON
df <- read.table(paste0(res_dir,"zonal_training_result_20171208.txt"))
...
write.csv(df3,paste0(res_dir,"map_vs_ref_val_poly_20171208.csv"),row.names = F)
...
write.csv(areas_merge,paste0(res_dir,"areas_l2_20171208.csv"),row.names = F)
